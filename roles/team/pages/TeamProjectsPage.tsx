 ...
 * Team Projects Page
 * 
 * Re-exports FreelancerProjectsPage with Team-specific user context.
 * This allows sharing the same UI while using Team mock data.
 * 
 * Future: Add team-specific features like member assignment, shared comments.
 */
...
export const TeamProjectsPage: React.FC<TeamProjectsPageProps> = ({ onLogout, onNavigate }) => {
    // State
    ...
    const handleOpenProject = (projectId: string) => {
        console.log("Open Team Project Workspace", projectId);
        onNavigate('project-detail');
    };

    const handleDeliver = (projectId: string) => {
        console.log("Deliver Team Project", projectId);
        onNavigate('delivery');
    };

    return (
        <DashboardLayout user={MOCK_TEAM_USER} onLogout={onLogout} onNavigate={onNavigate} activePage="projects">
            <div className="max-w-7xl mx-auto py-4 md:py-8 space-y-8 min-h-[calc(100vh-140px)] flex flex-col">

                {/* --- A. PAGE HEADER --- */}
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-1">Team Projects</h1>
                        <p className="text-slate-400 text-sm">Manage your team's collaborative projects.</p>
                    </div>
