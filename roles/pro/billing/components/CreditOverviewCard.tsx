...
export const CreditOverviewCard = ({ balance, spent7d, avgDaily, onTopUp }: CreditOverviewCardProps) => {
  return (
    <div className="bg-gradient-to-br from-[#1a1b26] to-[#0f0f12] border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
      {/* Glow */}
      ...
      <div className="flex-1 w-full md:w-auto">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Available Credits</h2>
        </div>
        <div className="flex items-baseline gap-2">
          <span className="text-4xl md:text-5xl font-extrabold text-white">{balance}</span>
      ...
      <div className="flex gap-8 md:gap-12 w-full md:w-auto">
        <div>
          ...
          <p className="text-xl font-bold text-white">{spent7d}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500 font-medium mb-1">Daily Avg</p>
          <p className="text-xl font-bold text-white">~{avgDaily}</p>
        </div>
      </div>
