...
export const CreditLogTable = ({ transactions }: { transactions: CreditTransaction[] }) => {
  return (
    <div className="bg-[#1a1b26] border border-white/5 rounded-2xl overflow-hidden">
            ...
            {transactions.map((tx) => (
              <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                ...
                <td className="p-4">
                  ...
                  {tx.metadata && <p className="text-[10px] text-slate-500 mt-0.5 group-hover:text-slate-400">{tx.metadata}</p>}
                </td>
                <td className="p-4 text-[10px] font-bold uppercase tracking-wider">
                  {getCategoryBadge(tx.category)}
                </td>
                <td className={`p-4 text-right font-mono font-bold ${tx.amount > 0 ? 'text-green-400' : 'text-slate-200'}`}>
      ...
      <div className="p-3 text-center bg-black/20 border-t border-white/5">
        <button className="text-xs text-slate-500 hover:text-white transition-colors">Load older records</button>
      </div>
    </div>
  );
};
